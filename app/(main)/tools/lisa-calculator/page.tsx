...
const MAX_MONTHLY      = 333          // floor of £4,000 / 12
...
export default function LISACalculatorPage() {
  ...
  // Contribution years: for retirement, contribute until 50; for home, use slider
  const contribYears    = goal === 'retirement' ? Math.max(0, CONTRIB_STOP_AGE - age) : years
  // After 50, money grows untouched until 60
  const growthOnlyYears = goal === 'retirement' ? 10 : 0
  const totalYears      = contribYears + growthOnlyYears
  const displayYears    = goal === 'retirement' ? totalYears : years
  ...
  const data = useMemo(() => {
    ...
    for (let y = 1; y <= totalYears; y++) {
      ...
      if (r === 0) {
        ...
        const n  = 12
        const em = isContrib
          ? monthly + Math.min(monthly * BONUS_RATE, MAX_BONUS_ANNUAL / 12)
          : 0
            ...
            <div className="calc-inputs">
              ...
              <div>
                ...
                <div className="lisa-goal-toggle">
                  <button
                    className={`lisa-goal-btn${goal === 'home' ? ' lisa-goal-btn--active' : ''}`}
                    onClick={() => setGoal('home')}
                  >
                    🏠 First home
                  </button>
                  <button
                    className={`lisa-goal-btn${goal === 'retirement' ? ' lisa-goal-btn--active' : ''}`}
                    onClick={() => setGoal('retirement')}
                  >
                    🌅 Retirement
                  </button>
                </div>
              ...
              {goal === 'retirement' && (
                <div className="lisa-info-box">
                  <div className="lisa-info-box__row">
                    <span>Contributing until</span>
                    <strong>age {CONTRIB_STOP_AGE}</strong>
                  </div>
                  <div className="lisa-info-box__row">
                    <span>Growth-only period</span>
                    <strong>age {CONTRIB_STOP_AGE}–{RETIRE_AGE}</strong>
                  </div>
                  <div className="lisa-info-box__row">
                    <span>Can access at</span>
                    <strong>age {RETIRE_AGE} ({totalYears} yrs away)</strong>
                  </div>
                </div>
              )}
            ...
            <div className="calc-results">
              ...
              <div className="calc-summary">
                <div className="calc-summary__main">
                  <div className="calc-summary__label">
                    {goal === 'retirement'
                      ? `Total LISA value at age ${RETIRE_AGE}`
                      : `Total LISA value after ${years} year${years !== 1 ? 's' : ''}`}
                  </div>
              ...
              <div className="calc-chart-wrap">
                <div className="lisa-breakdown-bar">
                  {totalContrib > 0 && (
                    <div
                      className="lisa-breakdown-bar__seg lisa-breakdown-bar__seg--contrib"
                      style={{ width: `${(totalContrib / Math.max(final.value, 1)) * 100}%` }}
                    />
                  )}
                  {totalBonus > 0 && (
                    <div
                      className="lisa-breakdown-bar__seg lisa-breakdown-bar__seg--bonus"
                      style={{ width: `${(totalBonus / Math.max(final.value, 1)) * 100}%` }}
                    />
                  )}
                  {totalGrowth > 0 && (
                    <div
                      className="lisa-breakdown-bar__seg lisa-breakdown-bar__seg--growth"
                      style={{ width: `${(totalGrowth / Math.max(final.value, 1)) * 100}%` }}
                    />
                  )}
                </div>
                <div className="lisa-breakdown-legend">
                  <span>
                    <span className="legend-dot legend-dot--grey" />
                    Your money{' '}
                    <strong>{Math.round((totalContrib / Math.max(final.value, 1)) * 100)}%</strong>
                  </span>
                  <span>
                    <span className="legend-dot legend-dot--green" />
                    Gov. bonus{' '}
                    <strong>{Math.round((totalBonus / Math.max(final.value, 1)) * 100)}%</strong>
                  </span>
                  {totalGrowth > 0 && (
                    <span>
                      <span className="legend-dot" style={{ background: 'var(--green)' }} />
                      Growth{' '}
                      <strong>{Math.round((totalGrowth / Math.max(final.value, 1)) * 100)}%</strong>
                    </span>
                  )}
                </div>
                  ...
                  {chartPoints.map((pt, i) => {
                    ...
                    return (
                      <g key={pt.year}>
                        ...
                        {bonusH > 0 && (
                          <rect x={x} y={CHART_H - contribH - bonusH} width={barW} height={bonusH} fill="#4caf82" rx="2" />
                        )}
                        {growthH > 0 && (
                          <rect x={x} y={CHART_H - contribH - bonusH - growthH} width={barW} height={growthH} fill="#2e7d5e" rx="2" />
                        )}
                        {(i === 0 || i === chartPoints.length - 1 || i % labelEvery === 0) && (
                          <text x={x + barW / 2} y={CHART_H + 14} textAnchor="middle" fontSize="9" fill="#9e9e9a">
                            {pt.year}y
                          </text>
                        )}
                      </g>
              ...
              <div className="calc-insight">
                ...
                <p>
                  {goal === 'home' ? (
                    <>
                      Over {years} year{years !== 1 ? 's' : ''}, you&apos;ll earn{' '}
                      <strong style={{ color: 'var(--green)' }}>{fmt(totalBonus)}</strong>{' '}in government
                      bonuses — on top of every penny you put in. That&apos;s{' '}
                      {fmt(MAX_BONUS_ANNUAL)} per year of free money, just for saving.
                    </>
                  ) : (
                    <>
                      You contribute until age {CONTRIB_STOP_AGE}, earning{' '}
                      <strong style={{ color: 'var(--green)' }}>{fmt(totalBonus)}</strong> in bonuses.
                      Your pot then grows untouched for 10 years until you can access it at age {RETIRE_AGE}.
                    </>
                  )}
                </p>
              </div>
              ...
              {goal === 'home' && (
                <div className="calc-insight" style={{ background: '#fff8e6', borderColor: 'rgba(245,158,11,0.25)' }}>
                  <span className="calc-insight__icon">🏠</span>
                  <p>
                    Your LISA can be used toward any UK first home worth up to{' '}
                    <strong>£450,000</strong>. The bonus is paid directly to your conveyancer at completion — you never receive it as cash.
                  </p>
                </div>
              )}
              ...
              <div className="tool-signup-prompt__left">
                ...
                <div className="tool-signup-prompt__courses">
                  ...
                  <Link href="/courses/pensions-and-your-future" className="tool-prompt-course">
                    <span>🔮</span>
                    <div>
                      <div className="tool-prompt-course__title">Pensions &amp; Your Future</div>
                      <div className="tool-prompt-course__meta">9 lessons · Free</div>
                    </div>
                    <span className="tool-prompt-course__arrow">→</span>
                  </Link>
                </div>
